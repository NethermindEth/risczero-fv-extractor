...
function cumulative_wp_proof(part: number, ir: IR.Statement[], linesPerPart: number, partDrops: IR.DropFelt[][], firstPass: boolean): string {
    ...
    `    try simplify_get_hack`,
		...
		`    ${dropCount === 0 ? "-- " : ""}try simp only [State.drop_update_swap, State.drop_update_same]`,
		`    ${dropCount === 0 ? "-- " : ""}rewrite [State.dropFelts]`,
		`    ${dropCount === 0 ? "-- " : ""}try simp only [State.dropFelts_buffers, State.dropFelts_bufferWidths, State.dropFelts_cycle, State.dropFelts_felts, State.dropFelts_isFailed, State.dropFelts_props, State.dropFelts_vars]`,
		`    ${dropCount === 0 ? "-- " : ""}try simp only [Map.drop_base, ne_eq, Map.update_drop_swap, Map.update_drop]`,
		`    -- ${setCount} set${setCount === 1 ? "" : "s"}`,
		`    ${setCount === 0 ? "-- " : ""}rewrite [Map.drop_of_updates]`,
		`    ${setCount === 0 ? "-- " : ""}try simp only [Map.drop_base, ne_eq, Map.update_drop_swap, Map.update_drop]`,
...
function extractStateTransformers(stdout: string, funcName: string, part: number): [stateTransformer: string, cumulativeTransformer: string] {
  console.log(`RECEIVED(witness)-------------\n${stdout}`)
	...
	const secondErrorStart = stdout.split("\n").slice(firstErrorStart+1).findIndex(line => line.includes(`${funcName}/Witness/WeakestPresPart${part}.lean:`) && line.includes("error:")) + firstErrorStart + 1;
